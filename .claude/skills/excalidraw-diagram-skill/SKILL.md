...
Check for `playwright-cli` first — if it's on PATH, use it. It drives an
already-installed browser, so there is nothing to download:
...
```bash
.claude/skills/excalidraw-diagram/references/render_with_playwright_cli.sh <path-to-file.excalidraw>
```

**Otherwise, fall back to Playwright's own bundled Chromium:**
```bash
cd .claude/skills/excalidraw-diagram/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```

Both output a PNG next to the `.excalidraw` file. Then use the **Read tool** on the PNG to actually view it.
...
### First-Time Setup
Check for `playwright-cli` **before** installing anything:

```bash
command -v playwright-cli
```

- **Found** — nothing to set up. `render_with_playwright_cli.sh` drives that
  existing browser directly; skip the steps below entirely.
- **Not found** — set up Playwright's own bundled Chromium:
  ```bash
